import {type ThumbnailStyle, thumbnailStyles} from "../assets/assets.ts";
...
const StyleSelector = ({value, onChange, isOpen, setIsOpen}:{value:ThumbnailStyle; onChange:(style:ThumbnailStyle)=>void; isOpen:boolean;setIsOpen:(open:boolean)=>void}) => {
    ...
    return (
        <div className='relative space-y-3 dark'>
            ...
            {isOpen && (
                <div className='absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/12 backdrop-blur-3xl shadow-lg'>
                    {thumbnailStyles.map((style)=>(
                        <button key={style} onClick={()=> {onChange(style); setIsOpen(false)}} className="flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30">
